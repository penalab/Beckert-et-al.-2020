...
%
% For questions and/or comments please contact me at "thomas.kreuz (at) cnr.it".  

function [es,ed]=Event_Sync(x_spikes,y_spikes)

if isempty(x_spikes) && isempty(y_spikes)
    es = 0;
    ed = 0;
    return
end

x_spikes(x_spikes < 0.001) = 0.001;
y_spikes(y_spikes < 0.001) = 0.001;
...
precision=3;            % - logarithm_10 ( sampling interval )

taumode=1;             % how is the allowed time lag defined?            (1-fixed interval (tau),2-adaptive)
timemode=3;            % cumulative, windowed or cumulative normalized?  (1-cumulative,2-windowed,3:cumulative normalized)

tau=3;                  % maximum time delay for which events are still considered to be synchronous (for taumode=1)
window=1000;              % window size (for timemode=2)
fs=16;

printmode=0;            % saving to postscript file?            (0-no,1-yes)
filename=['Event-Sync-Alice',num2str(timemode),'.ps'];  % name of this postscript file
