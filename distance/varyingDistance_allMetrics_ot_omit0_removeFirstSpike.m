        ...
        for unit = 2:length(data{date}{site})
            ...
            for file=1:length(filetypes)
                ...
                for r = 1:size(TrialData, 1)
                    for c = 1:size(TrialData, 2)
                        if ~isempty(TrialData{r, c})
                            TrialData{r,c}(1) = [];
                        end
                    end
                end                
...
%                             [holder3(t1, t2), ~] = Event_Sync(TrialData{u, t1}, TrialData{u, t2});
                            holderM(t1, t2) = (length(TrialData{u, t1}) + length(TrialData{u, t2})) / 2;
                            end
                        end
                    end
                    ...
                    Mean{count}{file}(u) = nanmean(holderM(idx));
                end
...
% cd('K:\Data\Analysis\ReproducibilityPaper\working')
cd('K:\WorkingFolder')

save otFF_distance_100_omit_removeFirstSpike
% %
cd('K:\WorkingFolder')
load('K:\WorkingFolder\tetrodekeys\SpeakerIndex.mat','SpeakerIndex')

for t = 1:2
    load('otFF_distance_100_omit_removeFirstSpike.mat');
    ...
    switch t
        case 1
            save otAZ_distance_100_omit_removeFirstSpike distanceES distanceEuc distanceVP Mean
        case 2
            save otEL_distance_100_omit_removeFirstSpike distanceES distanceEuc distanceVP Mean
    end
