...
def reproducibility(SNR):
    ...
    N=100                   # number of neurons simultaneously simulated
    duration=0.5*second     # duration of one simulation, 200 seconds in the paper
#    tau_noise=5*ms
    tau_noise = 10*ms
    input=NeuronGroup(1,model='dx/dt=-x/tau_noise+(2./tau_noise)**.5*xi:1', method='euler')
    ...
    I : 1 (linked)
    '''
    neurons=NeuronGroup(N, model=eqs_neurons,threshold='x > 1',reset='x = 0',refractory=5*ms, method='euler')
    neurons.x=rand(N) # random initial conditions
    neurons.I=linked_var(input,'x')
    rate=PopulationRateMonitor(neurons) # PSTH
    sp = SpikeMonitor(neurons)
...
#    print('running')
    return timescale,strength,sp,spt,mean(rate.rate),rate.rate

if __name__=='__main__':
    #pool = multiprocessing.Pool(multiprocessing.cpu_count()-1) # all cores but one
    #SNRdB= linspace(5,15,10) # 100 points in the paper
    SNRdB = 8
    SNR = 10.**(SNRdB/10.)
    #results = pool.map(reproducibility, SNR) # launches multiple processes
    #results = map(reproducibility, SNR)    
    results = reproducibility(SNR)
    #timescale,strength=zip(*results)
    timescale,strength,sp,spt,m,r = results
