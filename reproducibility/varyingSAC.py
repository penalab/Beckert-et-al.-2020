...
def peak_integral(SAC, int):
    repro = np.sum(SAC[len(SAC)/2-int:len(SAC)/2+int])
    return repro    
...
for idx, h in enumerate(hist):
    ...
    for p in h:
        try:
            ...
            SAC = SAC * br.Hz ** 2
            sacnorm_j, strength_j = jorisnorm(SAC, reps, np.mean(p), dur, cw)
            sacnorm_b, strength_b = brettenorm(SAC, np.mean(p), cw)
            ...
            pass
        r_brette[idx].append(sacnorm_b)
        r_joris[idx].append(sacnorm_j)
    
    
del cw, mw_time, maxWidth, bins, idx, h, p, t, SAC, strength_j, strength_b, dur, hist, reps, sacnorm_b, sacnorm_j
