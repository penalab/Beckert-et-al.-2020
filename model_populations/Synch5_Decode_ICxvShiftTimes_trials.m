function Synch5_Decode_ICxShiftTime_trials_pipeline(repet, ITD, dir_name, SNR_ICcl, SNR_ICx, ICx_side, ICx_weight, ICx_width, freq_width)


eval(['load ' dir_name 'SynchResults\data\icx\SynchICx_ITD' num2str(ITD) '_' r_txt ...
    ...
    '_shiftshuff CF BITD s BW SNR_ICcl bestITD bestF We Wi SNR_ICx NP tau_syn ICx_weight ICx_side ICx_width freq_width'])
...
eval(['load ' dir_name 'SynchResults\data\iccl\SynchICcl_ITD' num2str(ITD) '_001_SNR20 T'])
eval(['load ' dir_name 'SynchResults\data\SynchParms_reducedSNR bestITD bestF NP'])
...
for n = 1:Nn
    %works with tau = .1
    W(n,:) = 20*exp(-.5*((bestITD - bestITD(n))/30).^2); 
...
eval(['load ' dir_name 'SynchResults\data\icx\SynchICx_ITD' num2str(ITD) '_001_SNR20_shiftshuff spikesICx Rate'])
