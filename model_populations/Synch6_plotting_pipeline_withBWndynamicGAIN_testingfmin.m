cd('K:\python\model_icx_codeonly\SynchResults\data\testingvars_BWnGainGeGi_fmin_Wweight_150-250time')
...
for f = 1:size(files, 1)
    files(f, regexp(files(f,:),'[modeloutITDSNRICclSNRICxICxweightICxsideICxwidthfreqwidthshiftshuffbwgainLUfminWweightbin.mat ]')) = '_';
...
for ITD = itds
    ...
    if isfile([dir_name 'SynchResults\data\testingvars_BWnGainGeGi_fmin_Wweight_150-250time\modelout_ITD' num2str(ITD) '_' r_txt ...
...
for p = 91:95 %:size(params_used, 1)
...
BIN = params_used(p, 14);
    ...
    eval(['load ' dir_name 'SynchResults\data\testingvars_BWnGainGeGi_fmin_Wweight_150-250time\modelout_ITD' num2str(ITD) '_' r_txt ...
        '_SNRICcl' num2str(SNR_ICcl * 10) ...
        '_SNRICx' num2str(SNR_ICx * 100000) ...
        '_ICxweight' num2str(ICx_weight * 10) ...
        '_ICxside' num2str(ICx_side * 10) ...
...
sidepeaks(p, 1) = max(SC(1:41));
sidepeaks(p, 2) = max(SC2(1:41));
