...
%         Witd = exp(-.5*((bestITD(k) - BITD)/sITD).^2) - .5*exp(-.5*((bestITD(k)+half_p(k) - BITD)/(sITD)).^2) - .5*exp(-.5*((bestITD(k)-half_p(k) - BITD)/(sITD)).^2);
        % test weights
        Witd = exp(-.5*((bestITD(k) - BITD)/sITD).^2) + 3*exp(-3*((bestITD(k)+half_p(k) - BITD)/(sITD)).^2) + 3*exp(-3*((bestITD(k)-half_p(k) - BITD)/(sITD)).^2);
...
eval(['save ' dir_name 'SynchResults/data/SynchParms_SNR' num2str(SNR_ICcl * 10) ' CF BITD s BW SNR_ICcl bestITD bestF We Wi SNR_ICx NP tau_syn'])
