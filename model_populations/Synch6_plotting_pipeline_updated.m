...
for f = 1:size(files, 1)
    files(f, regexp(files(f,:),'[modeloutITDSNRICclSNRICxICxweightICxsideICxwidthfreqwidthshiftshuffbwgainLUfminWweightbinVTVisyncalc.mat ]')) = '_';
...
for ITD = itds
    ...
    if isfile([dir_name 'SynchResults\data\out_synch\modelout_ITD' num2str(ITD) '_' r_txt ...
...
sidepeaks(p, 1) = max(M(ind2));
sidepeaks(p, 2) = max(M2(ind2));
