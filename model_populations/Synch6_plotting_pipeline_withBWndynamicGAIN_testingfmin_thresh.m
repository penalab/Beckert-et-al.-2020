cd('K:\python\model_icx_codeonly\SynchResults\data\testingvars')
...
for f = 1:size(files, 1)
    files(f, regexp(files(f,:),'[modeloutITDSNRICclSNRICxICxweightICxsideICxwidthfreqwidthshiftshuffbwgainLUfminWweightbinVTVi.mat ]')) = '_';
    ...
    params{f}(14) = params{f}(14) * 10;
    params{f}(15) = params{f}(15) * 10;
    params{f}(16) = params{f}(16) * 10;
end
...
fmin = unique(params(:, 12))';
wweight = unique(params(:, 13))';
bin = unique(params(:, 14))';
vt = unique(params(:, 15))';
vi = unique(params(:, 16))';
...
for ITD = itds
    ...
    if isfile([dir_name 'SynchResults\data\testingvars\modelout_ITD' num2str(ITD) '_' r_txt ...
    ...
    params_used(L, 12) = FMIN;
    params_used(L, 13) = W_weight;
    params_used(L, 14) = BIN;
    params_used(L, 15) = VT;
    params_used(L, 16) = Vi;
...
sidepeaks = nan(size(params_used, 1), 2);
...
FMIN = params_used(p, 12);
W_weight = params_used(p, 13);
BIN = params_used(p, 14);
...
Vi = params_used(p, 16);
    ...
    eval(['load ' dir_name 'SynchResults\data\testingvars\modelout_ITD' num2str(ITD) '_' r_txt ...
        '_SNRICcl' num2str(SNR_ICcl * 10) ...
        '_SNRICx' num2str(SNR_ICx * 100000) ...
        '_ICxweight' num2str(ICx_weight * 10) ...
        '_ICxside' num2str(ICx_side * 10) ...
...
sidepeaks(p, 1) = max(SC(ind2));
sidepeaks(p, 2) = max(SC2(ind2));
...
sps = sidepeaks(:, 1) - sidepeaks(:, 2);
sps_m = reshape(sps, 5, length(sps)/5);

figure
plot(mean(sps_m, 1))
xlabel('settings group')
